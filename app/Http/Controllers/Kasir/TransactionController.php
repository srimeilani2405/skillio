...
class TransactionController extends Controller
{
    ...
    public function storeCustomer(Request $request)
    {
        try {
            ...
            LogActivity::log('Kasir menambahkan customer baru: ' . $customer->nama, 'Tambah');
    ...
    public function store(Request $request)
    {
        try {
            ...
            if (!isset($data['paket']) || empty($data['paket'])) {
                return response()->json(['success' => false, 'message' => 'Pilih minimal 1 paket kursus']);
            }
            if (!isset($data['id_customer']) || empty($data['id_customer'])) {
                return response()->json(['success' => false, 'message' => 'Pilih customer terlebih dahulu']);
            }
            if (!isset($data['uang_bayar']) || $data['uang_bayar'] <= 0) {
                return response()->json(['success' => false, 'message' => 'Masukkan jumlah uang bayar']);
            }
            ...
            foreach ($data['paket'] as $item) {
                ...
                if (!$course) {
                    return response()->json(['success' => false, 'message' => 'Paket kursus tidak ditemukan']);
                }

                $jumlahBeli = $item['jumlah'] ?? 1;

                if ($course->sisaKuota() < $jumlahBeli) {
                    return response()->json([
                        'success' => false,
                        'message' => 'Kuota kursus "' . $course->nama . '" tidak mencukupi. Sisa kuota: ' . $course->sisaKuota(),
                    ]);
                }

                $hargaPaket   = $course->harga + ($course->biaya_sertifikat ?? 0);
                $totalHarga  += $hargaPaket * $jumlahBeli;
                $namaKursus[] = $course->nama;
                $courseList[] = $course;
                ...
                $detailItems[] = [
                    ...
                    'jumlah'       => $jumlahBeli,
                    'subtotal'     => $hargaPaket * $jumlahBeli,
                ];
            }
            ...
            // ← FIX: Ambil id user dengan aman, pastikan integer
            $authUser = Auth::user();
            $userId   = $authUser ? (int) $authUser->id : 1;

            // Fallback: cari dari DB berdasarkan email jika id tidak valid
            if ($userId <= 0 && $authUser) {
                $userFromDb = User::where('email', $authUser->email)->first();
                $userId     = $userFromDb ? (int) $userFromDb->id : 1;
            }
            ...
            // Cek dan update status kursus jika kuota habis
            ...
            LogActivity::log(
                'Kasir menambahkan transaksi baru (' . $kodeTransaksi . ') untuk customer ' .
                ($customer->nama ?? '-') . ' - Kursus: ' . implode(', ', $namaKursus) .
    ...
    {
        $transaksi = Transaction::with(['customer', 'details.course'])->findOrFail($id);
