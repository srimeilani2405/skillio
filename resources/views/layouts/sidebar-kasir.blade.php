...
<head>
    ...
    <title>@yield('title', 'Skillio Kasir')</title>
...
<body>
    ...
    <div class="sidebar">
        ...
        <a href="{{ url('/kasir/dashboard') }}" class="{{ request()->is('kasir/dashboard') ? 'active' : '' }}">
            <i class="fas fa-home"></i> Dashboard
        </a>

        <a href="{{ url('/kasir/transactions') }}" class="{{ request()->is('kasir/transactions*') ? 'active' : '' }}">
        ...
        <div style="padding: 20px;">
            ...
            <form id="logout-form-kasir" action="{{ url('/logout') }}" method="POST" style="display:none;">
                @csrf
            </form>
        </div>
    </div>
    ...
    <div id="content-wrapper">
        <div class="topbar">
            <div><b>Kasir Panel</b></div>
            <div><b>{{ Auth::user()->name ?? 'Kasir' }}</b></div>
        </div>
