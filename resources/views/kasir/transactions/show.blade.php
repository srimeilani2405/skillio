...
<div id="kuitansi">
    ...
    <div class="jadwal-box">
        <strong><i class="fas fa-calendar-alt"></i> JADWAL KURSUS:</strong><br>
        <div style="margin-top: 5px;">
            📅 <strong>Hari:</strong> {{ $hariString }}<br>
            🕐 <strong>Jam:</strong> {{ $jamMulai }} - {{ $jamSelesai }} WIB<br>
            📆 <strong>Periode Kursus:</strong> {{ $tanggalMulai->translatedFormat('d F Y') }} s/d {{ $tanggalSelesai->translatedFormat('d F Y') }}<br>
            ⏱️ <strong>Durasi per Sesi:</strong> {{ $course->durasi_per_sesi ?? '-' }} menit
        </div>
    </div>
    @endif
    ...
    <div class="deskripsi-box">
        <strong><i class="fas fa-info-circle"></i> DESKRIPSI KURSUS:</strong><br>
    ...
    <div style="font-size: 11px; margin-top: 8px; font-style: italic;">
        Terbilang: <strong>{{ terbilang($transaksi->total_harga ?? 0) }} rupiah</strong>
    </div>

    {{-- CATATAN PENTING --}}
    <div style="font-size: 10px; margin-top: 15px; border-top: 1px dashed #ccc; padding-top: 8px; color: #666;">
        <strong>📌 Catatan:</strong> Kuitansi ini berlaku sebagai bukti pembayaran yang sah. Harap disimpan dengan baik.
    </div>

    {{-- TANGGAL KOTA --}}
    <div style="text-align:right; font-size:12px; margin-top: 16px;">
