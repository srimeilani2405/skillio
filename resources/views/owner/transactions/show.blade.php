...
<style>
    ...
    .section-header.blue  { background: #2196f3; }
    .section-header.green { background: #43a047; }
    .section-header.cyan  { background: linear-gradient(90deg, #26c6da 0%, #00acc1 100%); }
    ...
    .status-lunas { background: #43a047; color: #fff; }
    ...
    @media print {
        ...
        #print-kuitansi,
        #print-kuitansi * { visibility: visible; }
...
<style media="print">
    ...
    #print-kuitansi .header-title { text-align: center; margin-bottom: 18px; }
    #print-kuitansi .header-title h3 { font-size: 18px; font-weight: bold; margin: 0; letter-spacing: 1px; }
    #print-kuitansi .header-title h2 { font-size: 22px; font-weight: 900; text-transform: uppercase; letter-spacing: 2px; margin: 4px 0 0; }
    ...
    #print-kuitansi .info-item { display: flex; gap: 4px; }
    #print-kuitansi .info-label { font-weight: bold; white-space: nowrap; min-width: 130px; }
    ...
    #print-kuitansi table th { text-align: center; font-weight: bold; }
    #print-kuitansi table td:last-child { text-align: right; }
    ...
    #print-kuitansi .payment-info { margin: 15px 0; font-size: 12px; }
    #print-kuitansi .payment-info p { margin: 5px 0; }

    #print-kuitansi .terbilang { margin: 15px 0; font-size: 11px; font-style: italic; }

    #print-kuitansi .ttd-section { text-align: right; margin-top: 40px; font-size: 12px; }
    ...
    #print-kuitansi .ttd-section .ttd-box:first-child { margin-right: 30px; margin-left: 0; }

    hr { margin: 12px 0; border: none; border-top: 1px solid #333; }
</style>
@endpush
...
<div class="d-flex justify-content-between align-items-center mb-4 no-print">
    ...
    <div class="d-flex gap-2">
        ...
        <a href="{{ url('/owner/transactions') }}" class="btn btn-outline-secondary">
        ...
        <div class="section-box">
            ...
            <div class="section-body">
                <div class="info-row">
                    ...
                    <span class="info-value">: <strong>{{ $transaction->kode_transaksi }}</strong></span>
                </div>
                <div class="info-row">
                ...
                <div class="info-row">
                    ...
                    <span class="info-value">: {{ $transaction->customer->nama ?? '-' }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">No. Telepon</span>
                    <span class="info-value">: {{ $transaction->customer->no_telp ?? '-' }}</span>
                </div>
                <div class="info-row">
        ...
        <div class="section-box">
            ...
            <div class="section-body">
                <div class="info-row">
                    ...
                    <span class="info-value">: <strong>Rp {{ number_format($transaction->total_harga, 0, ',', '.') }}</strong></span>
                </div>
                <div class="info-row">
                    <span class="info-label">Uang Bayar</span>
                    <span class="info-value">: Rp {{ number_format($transaction->uang_bayar, 0, ',', '.') }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Uang Kembali</span>
                    <span class="info-value">: Rp {{ number_format($transaction->uang_kembali, 0, ',', '.') }}</span>
                </div>
                <div class="info-row">
    ...
    <div style="max-width: 680px; margin: 0 auto;">
        ...
        @php
            ...
            if($course) {
                ...
                $tanggalMulai = \Carbon\Carbon::parse($transaction->created_at);
        ...
        <table>
            ...
            <tbody>
                ...
                <tr style="border-top: 2px solid #333;">
                    ...
                    <td style="font-weight:bold;">Rp {{ number_format($transaction->total_harga, 0, ',', '.') }}</td>
                </tr>
            </tbody>
        </table>

        <div class="payment-info">
            <p><strong>METODE PEMBAYARAN : {{ strtoupper($transaction->metode_pembayaran ?? 'TUNAI') }}</strong></p>
            <p><strong>UANG BAYAR : Rp {{ number_format($transaction->uang_bayar, 0, ',', '.') }}</strong></p>
            <p><strong>UANG KEMBALI : Rp {{ number_format($transaction->uang_kembali, 0, ',', '.') }}</strong></p>
        </div>

        <div class="terbilang">
            <strong>Terbilang:</strong> {{ terbilang($transaction->total_harga) }} rupiah
        </div>

        <div style="text-align:right; margin-top: 20px;">
            Subang, {{ \Carbon\Carbon::parse($transaction->created_at)->translatedFormat('d F Y') }}
        </div>

        <div class="ttd-section">
            <div class="ttd-box">
                <div style="margin-bottom: 50px;">( {{ $transaction->customer->nama ?? 'Peserta' }} )</div>
                <div>Peserta</div>
            </div>
            <div class="ttd-box">
                <div style="margin-bottom: 50px;">( {{ $transaction->user->name ?? 'Kasir' }} )</div>
